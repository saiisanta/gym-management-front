...
    setLoading(true);
    try {
      // 1. Encontrar el usuario actual ANTES de la actualización
      const existingUser = usuarios.find((u) => u.id === id);

      // ✅ CORRECCIÓN CLAVE: Asegurar que el PlanId se envía en el payload si no viene del formulario.
      const payloadToSend = { ...updatedData };

      // Si el formulario NO envió planId/PlanId (o lo envió como undefined),
      // pero el usuario ya tiene uno existente, lo incluimos en el payload.
      if (
        (payloadToSend.planId === undefined && payloadToSend.PlanId === undefined) &&
        existingUser?.planId !== undefined &&
        existingUser?.planId !== null
      ) {
        payloadToSend.planId = existingUser.planId;
        console.log(`[Update] PlanId (ID ${existingUser.planId}) agregado al payload para persistencia.`);
      } else if (
        // Si el formulario lo envió como null/0/vacío, respeta el cambio intencional del formulario
        payloadToSend.planId === null || payloadToSend.PlanId === null
      ) {
          console.log(`[Update] PlanId enviado como NULL. Se borrará en el Backend.`);
      }

      const raw = await apiUpdateUser(id, payloadToSend); // <-- Usar payloadToSend
